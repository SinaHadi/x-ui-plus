...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/MHSanaei/3x-ui1/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/mhsanaei/3x-ui1/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    systemctl disable x-ui1
    rm /etc/systemd/system/x-ui1.service -f
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/mhsanaei/3x-ui1/master/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    if [[ $twoFactorConfirm != "y" && $twoFactorConfirm != "Y" ]]; then
        /usr/local/x-ui1/x-ui1 setting -username ${config_account} -password ${config_password} -resetTwoFactor false >/dev/null 2>&1
    else
        /usr/local/x-ui1/x-ui1 setting -username ${config_account} -password ${config_password} -resetTwoFactor true >/dev/null 2>&1
        echo -e "Two factor authentication has been disabled."
    fi
    
    echo -e "Panel login username has been reset to: ${green} ${config_account} ${plain}"
    echo -e "Panel login password has been reset to: ${green} ${config_password} ${plain}"
    echo -e "${green} Please use the new login username and password to access the x-ui1 panel. Also remember them! ${plain}"
    confirm_restart
}
...
reset_webbasepath() {
    ...
    /usr/local/x-ui1/x-ui1 setting -webBasePath "${config_webBasePath}" >/dev/null 2>&1
...
reset_config() {
    ...
    /usr/local/x-ui1/x-ui1 setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(/usr/local/x-ui1/x-ui1 setting -show true)
    if [[ $? != 0 ]]; then
    ...
    local existing_cert=$(/usr/local/x-ui1/x-ui1 setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        /usr/local/x-ui1/x-ui1 setting -port ${port}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            LOGI "x-ui1 Started Successfully"
        else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "x-ui1 and xray stopped successfully"
        else
...
restart() {
    systemctl restart x-ui1
    sleep 2
    check_status
    if [[ $? == 0 ]]; then
        LOGI "x-ui1 and xray Restarted successfully"
    else
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "x-ui1 Set to boot automatically on startup successfully"
    else
        LOGE "x-ui1 Failed to set Autostart"
    fi
...
disable() {
    systemctl disable x-ui1
    if [[ $? == 0 ]]; then
        LOGI "x-ui1 Autostart Cancelled successfully"
    else
        LOGE "x-ui1 Failed to cancel autostart"
    fi
...
show_log() {
    ...
    0)
        ...
        ;;
    1)
        journalctl -u x-ui1 -e --no-pager -f -p debug
        if [[ $# == 0 ]]; then
            before_show_menu
        fi
        ;;
...
update_shell() {
    wget -O /usr/bin/x-ui1 -N https://github.com/MHSanaei/3x-ui1/raw/main/x-ui1.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_status() {
    if [[ ! -f /etc/systemd/system/x-ui1.service ]]; then
        return 2
    fi
    temp=$(systemctl status x-ui1 | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    temp=$(systemctl is-enabled x-ui1)
        ...
        if [ -z "$domains" ]; then
            ...
            if echo "$domains" | grep -qw "$domain"; then
                ...
                if [[ -f "${webCertFile}" && -f "${webKeyFile}" ]]; then
                    /usr/local/x-ui1/x-ui1 cert -webCert "$webCertFile" -webCertKey "$webKeyFile"
...
ssl_cert_issue() {
    ...
    *)
    ...
    reloadCmd="x-ui1 restart"

    LOGI "Default --reloadcmd for ACME is: ${yellow}x-ui1 restart"
    LOGI "This command will run on every certificate issue and renew."
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; x-ui1 restart"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
        1)
            LOGI "Reloadcmd is: systemctl reload nginx ; x-ui1 restart"
            reloadCmd="systemctl reload nginx ; x-ui1 restart"
            ;;
        2)  
            LOGD "It's recommended to put x-ui1 restart at the end, so it won't raise an error if other services fails"
            read -rp "Please enter your reloadcmd (example: systemctl reload nginx ; x-ui1 restart): " reloadCmd
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(/usr/local/x-ui1/x-ui1 setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(/usr/local/x-ui1/x-ui1 setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(/usr/local/x-ui1/x-ui1 setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(/usr/local/x-ui1/x-ui1 setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(/usr/local/x-ui1/x-ui1 setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    1)
        if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
            ...
            /usr/local/x-ui1/x-ui1 setting -listenIP "${config_listenIP}" >/dev/null 2>&1
        ...
        ;;
    2)
        /usr/local/x-ui1/x-ui1 setting -listenIP 0.0.0.0 >/dev/null 2>&1
...
show_menu() {
    echo -e "
╔────────────────────────────────────────────────╗
│   ${green}3x-ui1 Panel Management Script${plain}                │
